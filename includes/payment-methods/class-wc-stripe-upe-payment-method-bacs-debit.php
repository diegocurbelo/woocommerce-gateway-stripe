...
class WC_Stripe_UPE_Payment_Method_Bacs_Debit extends WC_Stripe_UPE_Payment_Method {
	...
	public function attach_payment_method_to_customer_ajax() {
		...
		try {
			...
			$this->create_payment_token_for_user($user_id, $pament_token);
			// tendriamos que eliminar el transient para evitar que se autoelimine
			// get_transient( self::PAYMENT_METHODS_TRANSIENT_KEY . $payment_method_type . $this->get_id() );
			xdebug_break();
			// get_transient( WC_Stripe_Customer::PAYMENT_METHODS_TRANSIENT_KEY . WC_Stripe_Payment_Methods::BACS_DEBIT . $stripe_user->get_id() );
